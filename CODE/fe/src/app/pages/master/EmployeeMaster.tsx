...
export function EmployeeMaster() {
  ...
  const openCreateModal = () => {
    if (!canCreate) return;
    const defaultRole =
      roles.find((role) => role.status === "Active" && role.isDefault) ??
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={closeModal}>
          <div className="w-full max-w-2xl bg-white rounded-xl border border-slate-200 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              ...
              <button type="button" onClick={closeModal} disabled={isSubmitting} className="p-1 rounded hover:bg-slate-100 text-slate-500 disabled:opacity-50">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
                ...
                <Select
                  ...
                  options={[
                    ...
                    ...roleOptions.map((role) => ({
                      value: role.id,
                      label: `${role.name}${role.isDefault ? " (Default)" : ""}${role.status === "Inactive" ? " (Inactive)" : ""}`
                    }))
                  ]}
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={closeModal} disabled={isSubmitting} className="px-4 py-2 border border-slate-300 rounded-md text-sm disabled:opacity-60">
                  Batal
                </button>
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-semibold disabled:opacity-60"
                >
                  {isSubmitting && <Loader2 className="mr-1.5 h-4 w-4 animate-spin" />}
                  {isSubmitting ? "Menyimpan..." : modalMode === "create" ? "Simpan Pegawai" : "Simpan Perubahan"}
                </button>
              </div>
            </form>
          </div>
